...
class ArrayHelper extends \yii\helpers\ArrayHelper
{
     ...
     * ---------------------------------------
     */
    public static function list_to_levellist($arr, $pk = 'id', $title__field = 'name', $pid_field = 'pid', $pid = 0, $level = 0) {
        static $list = [];
        foreach ($arr as $v) {
            if ($v[$pid_field] == $pid) {
                if ($level == 0)
                    ...
                    $list[$v[$pk]] = str_repeat("　", $level) . "|__" . $v[$title__field];
                self::list_to_levellist($arr, $pk, $title__field, $pid_field, $v[$pk], $level + 1);
            }
        }
        return $list;
    }
     ...
     * @return type
     */
    public static function getParents($ret, $nodeId, &$data) {
        foreach ($ret as $v) {
            if ($v['node_id'] == $nodeId) {
                if ($v['pid'] > 0) {
                    $data[$v['pid']] = (int) $v['pid'];
                    self::getParents($ret, $v['pid'], $data);
                }
            }
        }
    }
